...
public class CCA {
    ...
    public static Element Setup(String lambda, List<String> U) {
        Element delta1 = G1.newRandomElement().getImmutable();
        Element delta2 = G1.newRandomElement().getImmutable();
        Element delta3 = G1.newRandomElement().getImmutable();
    ...
    static CT Encrypt(Element M, MX Y) {
        ...
        C2 = C2.powZn(s);
        Element delta1 = params.get(params.size() - 3);
        Element delta2 = params.get(params.size() - 2);
        Element delta3 = params.get(params.size() - 1);
        // TODO:c的哈希如何计算
        byte[] cc = (Integer.toString(C0.hashCode()) + Integer.toString(C1.hashCode()) + Integer.toString(C2.hashCode())).getBytes();
        Element c = Zr.newElementFromHash(cc, 0, cc.length);
        Element C3 = ((delta1.powZn(c)).mul(delta2.powZn(r)).mul(delta3)).powZn(s);
        CT ct = new CT();
        ct.C0 = C0;
        ct.C1 = C1;
        ct.C2 = C2;
    ...
    static Element Decrypt(MX Y, List<List<Element>> sk, List<String> A, CT ct) {
        ...
        Element r1 = h.get(0);
        for (BigInteger j : SunionO) {
            r1 = r1.mul(h.get(j.intValue()));
        }
        if (!(bp.pairing(g, C2).isEqual(bp.pairing(C1, r1)))) {
            return null;
        }
        Element r2 = (delta1.powZn(c)).mul(delta2.powZn(r)).mul(delta3);
        if (!(bp.pairing(g, C3).isEqual(bp.pairing(C1, r2)))) {
            return null;
        }
        // 2
